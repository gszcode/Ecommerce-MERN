...
function Cart() {
	...
	const fetchDataCart = async () => {
		const dataResponse = await fetch(SummaryApi.viewProductCart.url, {
			method: SummaryApi.viewProductCart.method,
			credentials: "include",
			headers: {
		...
		if (dataApi.success) setData(dataApi.data);
	};
	...
	const increaseQuantity = async (id, qty) => {
		...
		if (responseData.success) fetchDataCart();
	};
									...
									<div className='px-4 py-2 relative'>
										...
										<div className='flex items-center justify-between'>
											<p className='text-red-600 font-medium text-lg'>
												{displayARGCurrency(product?.productId?.sellingPrice)}
											</p>
											<p className='text-slate-600 font-semibold text-lg'>
												{displayARGCurrency(product?.productId?.sellingPrice * product?.quantity)}
											</p>
										</div>
				...
				<div className='mt-5 lg:mt-0 w-full max-w-sm'>
					{loading ? (
						...
						<div className='h-36 bg-white'>
							...
							<div className='flex items-center justify-between px-4 gap-2 font-medium text-lg text-slate-600'>
								<p>Total Price</p>
								<p>{displayARGCurrency(totalPrice)}</p>
							</div>
